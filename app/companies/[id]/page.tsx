...
export default async function V1CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const category = getCategory(data, company.categoryId)
  const demands = getCompanyDemands(data, company.id)
  const related = relatedCompanies(data, company)

  return (
    <main className="v1-inner-page min-h-screen text-foreground">
      <V1Header />
      <section className="v1-inner-hero border-b border-primary/10 py-10">
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          ...
          <div className="mt-6 flex flex-col gap-5 md:flex-row md:items-center md:justify-between">
            ...
            <Link href="/contact"><Button className="h-10 gap-2 rounded-lg bg-gradient-to-r from-primary to-accent text-white hover:opacity-90"><Headphones className="size-4" />联系平台客服</Button></Link>
          </div>
        </div>
      </section>
      ...
      <section className="py-10">
        <div className="mx-auto grid max-w-7xl gap-6 px-4 md:px-6 lg:grid-cols-[1fr_360px]">
          ...
          <aside className="space-y-6">
            <div className="rounded-lg border border-primary/10 bg-card p-5 shadow-sm"><h2 className="font-bold">最新需求</h2><div className="mt-3 space-y-3">{demands.length > 0 ? demands.map((demand) => <Link key={demand.id} href={`/demands/${demand.id}`} className="v1-inner-panel-link block rounded-lg p-3"><p className="text-sm font-semibold text-foreground">{demand.title}</p><p className="mt-1 text-xs text-muted-foreground">{demand.publishedAt} · {demand.type}</p></Link>) : <p className="text-sm text-muted-foreground">暂无公开需求。</p>}</div></div>
            <div className="rounded-lg border border-primary/10 bg-card p-5 shadow-sm"><h2 className="font-bold">{data.site.copy.companyDetail.matchTitle}</h2><p className="mt-2 text-sm leading-6 text-muted-foreground">{data.site.copy.companyDetail.matchDescription}</p><Link href="/contact" className="mt-4 block"><Button className="h-10 w-full rounded-lg bg-gradient-to-r from-primary to-accent text-white hover:opacity-90">{data.site.contactButtonText}</Button></Link></div>
          </aside>
        </div>
      </section>
      {related.length > 0 ? <section className="bg-card py-10"><div className="mx-auto max-w-7xl px-4 md:px-6"><h2 className="mb-5 text-xl font-bold">同类企业</h2><div className="grid gap-5 lg:grid-cols-2">{related.map((item) => <V1CompanyCard key={item.id} company={item} category={getCategory(data, item.categoryId)} demands={getCompanyDemands(data, item.id)} />)}</div></div></section> : null}
