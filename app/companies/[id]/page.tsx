...
export default async function V1CompanyDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="min-h-screen bg-[#fff8f5] text-foreground">
      ...
      {related.length > 0 ? <section className="bg-white py-10"><div className="mx-auto max-w-7xl px-4 md:px-6"><h2 className="mb-5 text-xl font-bold">同类企业</h2><div className="grid gap-5 lg:grid-cols-2">{related.map((item) => <V1CompanyCard key={item.id} company={item} category={getCategory(data, item.categoryId)} demands={getCompanyDemands(data, item.id)} />)}</div></div></section> : null}
