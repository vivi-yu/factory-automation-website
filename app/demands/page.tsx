...
export default async function V1DemandsPage({ searchParams }: { searchParams: Promise<{ page?: string }> }) {
  ...
  const allDemands = latestDemands(data.demands)
      ...
      <section className="py-8 md:py-12">
        <div className="mx-auto max-w-7xl px-4 md:px-6">
          <div className="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
            {demands.map((demand) => (
              <V1ClickableCard key={demand.id} href={`/demands/${demand.id}`} className="group rounded-lg border border-border/40 bg-card p-5 shadow-sm transition-all hover:border-primary hover:shadow-xl hover:shadow-primary/10">
                ...
                <div className="mt-5 flex flex-wrap gap-2">
                  ...
                  <Link href="/contact"><Button className="h-9 rounded-lg bg-gradient-to-r from-primary to-accent text-white hover:opacity-90">联系客服</Button></Link>
                </div>
              </V1ClickableCard>
            ))}
          </div>
          ...
          <div className="mt-10 flex flex-wrap items-center justify-center gap-2">
            <PageLink disabled={safePage <= 1} href={`/demands?page=${safePage - 1}`} label="上一页" icon="prev" />
            {Array.from({ length: totalPages }).map((_, index) => {
              const itemPage = index + 1
              return <Link key={itemPage} href={`/demands?page=${itemPage}`} className={`flex size-10 items-center justify-center rounded-lg border text-sm font-medium ${itemPage === safePage ? 'border-primary bg-primary text-white' : 'border-border bg-card hover:border-primary'}`}>{itemPage}</Link>
            })}
            <PageLink disabled={safePage >= totalPages} href={`/demands?page=${safePage + 1}`} label="下一页" icon="next" />
          </div>
        </div>
      </section>
