            ...
            {results.demands.length > 0 ? (
              <div className="grid gap-4 md:grid-cols-2">
                {results.demands.map((demand) => (
                  <Link key={demand.id} href={`/demands/${demand.id}`} className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:border-blue-300 hover:shadow-md">
                    ...
                    <p className="mt-2 text-sm text-slate-500">{getCompanyName(data, demand.companyId)} · {demand.publishedAt}</p>
