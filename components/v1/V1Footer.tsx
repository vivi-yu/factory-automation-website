      ...
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-12 md:grid-cols-3 md:px-6">
        ...
        <div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 text-sm text-current/70">
            ...
            <Link href="/companies" className="hover:text-primary">企业</Link>
            <Link href="/demands" className="hover:text-primary">需求</Link>
            <Link href="/news" className="hover:text-primary">行业资讯</Link>
            <Link href="/contact" className="hover:text-primary">联系客服</Link>
            {categories.map((category) => <Link key={category.id} href={`/#category-${category.id}`} className="hover:text-primary">{category.name}</Link>)}
          </div>
        </div>
        <div>
          ...
          <Link href="/contact" className="mt-5 inline-block"><Button className="h-9 gap-2 rounded-lg text-white hover:opacity-90" style={{ background: 'var(--v1-button-bg)' }}><Headphones className="size-4" />联系平台客服</Button></Link>
        </div>
      </div>
    </footer>
  )
